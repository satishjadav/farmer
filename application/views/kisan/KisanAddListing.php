<?php 
$mrsperson = $this->session->userdata['adminperson'];
if(empty($mrsperson)){
    redirect(base_url() .'Crud/login');
}
// print_r($mrsperson);die;
if($mrsperson['role'] == '1' || $mrsperson['role'] == '2' || $mrsperson['role'] == '3'){
    if($mrsperson['role'] == '1'){
        $mrpersonus = $this->Main_model->getAll('kisan_info');
    }
    if($mrsperson['role'] == '2'){
        $querys = array( 'center_id'=>$mrsperson['username']);
        $mrperson = $this->Main_model->getAllWheresun('marketing_man',$querys); 
        $mrloop = array();       
        foreach($mrperson as $datasv){
        $kisaninfo = array( 'relation_creater'=>$datasv->mr_unid);
        $mrloop[] = $this->Main_model->getAllWheresun('kisan_info',$kisaninfo);    
    }
    // echo'<pre>';
    // print_r($mrpersonus);
    $mrpersonus = array();
    foreach($mrloop as $mrloops){
    ...
    if($mrsperson['role'] == '3'){
        $querys = array(
            'relation_creater'=>$mrsperson['username']
            // 'password'=>$mrsperson->password,
            // 'role'=>$mrsperson->role
        );
        $mrpersonus = $this->Main_model->getAllWheresun('kisan_info',$querys);
    }
...
<body>
    ...
    <div id="pcoded" class="pcoded body">        
	 ...
	 <?php  
         foreach ($mrpersonus as $val)  
         {  
           ?>
			<tr>  
            <td><?php echo $val->kisan_id;?></td>  
            <td><?php echo $val->kisan_full_name;?></td>  
            <td><?php echo $val->mobile_no;?></td>  
            <td><?php echo $val->full_address;?></td>  
            <td>
<a href="<?php echo base_url('Crud/showKisanDetail/').$val->kisan_id;?>" class="btn btn-warning btn-sm" style="border-radius: 12px;background-color: green;border: 1px solid darkgreen;">
 View Details
</a>
</td>
<td>
 <a class="btn btn-danger btn-sm" href="<?php echo base_url('Crud/deletedata/').$val->kisan_id;?>" style="border-radius: 12px;">Delete</a>&nbsp;
 <a class="btn btn-warning btn-sm" href="<?php echo base_url('Crud/update1/').$val->kisan_id;?>" style="border-radius: 12px;">Update</a>
</td>
	...
	<!-------- 	Update Author Modal  Add category Start----------- -->


					<div id="editcategory" class="modal fade"  tabindex="-1" role="dialog" style="padding-right: 17px;" aria-modal="true">
                        <div class="modal-dialog modal-lg">


                           <div class="modal-content">
                              <div class="modal-header bg-l-red py-1">
                                 <h5 class="modal-title">Update Purchase Category</h5>
                                 <button type="button" class="close text-light" data-dismiss="modal" aria-hidden="true">×</button>
                              </div>
                              <div class="modal-body">
          <form class="text-center m-4" style="color: #757575;" action="<?=base_url('Purchases/update_purchase_category')?>" method="post" id="editcategory_form" novalidate="novalidate">
                           <div class="form-row">
                              ...
                              <div class="col-md-4">
                                 <!-- First name -->
                                 <div class="md-form">
								 <input id="purchase_category_id" name="edit_id" type="hidden">
                                  <input id="purchase_category_name" type="text" name="purchase_category_name" class="form-control" placeholder="Enter Category Name" required="true" aria-required="true">
                                     <input id="purchase_category_des" type="text" name="purchase_category_des" class="form-control" placeholder="Enter Category Description" required="true" aria-required="true">
                                 </div> 
                              </div>

							<button type="submit" class="btn bg-l-blue ml-3 text-center"><i class="fa fa-check-square" aria-hidden="true"></i> Update</button>
                           </div>
						  </form> 
                        <!-- Form -->
						</div></div></div></div>										
			<!-------- 	Update Author Modal eND----------- -->
